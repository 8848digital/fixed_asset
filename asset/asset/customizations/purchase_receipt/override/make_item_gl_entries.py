...
def asset_make_item_gl_entries(self, gl_entries, warehouse_account=None):
	...
	def make_divisional_loss_gl_entry(item, outgoing_amount):
		...
		if divisional_loss:
			loss_account = (
				self.get_company_default("default_expense_account", ignore_validation=True) or stock_asset_rbnb
			)
